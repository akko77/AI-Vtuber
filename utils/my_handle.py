                ...
                for chunk in resp:
                    ...
                    if chat_type in ["chatgpt", "zhipu"]:
                        ...
                        if chat_type == "zhipu" and My_handle.config.get("zhipu", "model") == "智能体":
                            ...
                            # logger.info(f"查看流式回复chunk：{chunk}")
    ...
    def comment_handle(self, data):
        ...
        try:
            ...
            if My_handle.config.get("database", "comment_enable"):
                ...
                '''
                newid = self.db.execute(insert_data_sql, (username, content, datetime.now()))
            ...
            if chat_type in self.chat_type_list:
                ...
                if "stream" in self.config.get(chat_type) and self.config.get(chat_type, "stream"):
                    ...
                    # 记录数据库，把回复内容记录
                    if My_handle.config.get("database", "comment_enable") and newid is not None:
                        update_data_sql = '''
                            UPDATE danmu 
                            SET reply=?
                            WHERE id=?
                        '''
                        self.db.execute(update_data_sql, (resp_content,newid))
...
.....................................................................................................................
    ...
    def talk_handle(self, data):
        ...
        try:
            ...
            if 1==0:
                #进行情感分类，选择相应的参考音频
                sense = self.chatgpt.sense_last
                logger.info(f"sense接收：{sense}")
                if sense == "疑惑":
                    ref = "D:\\software\\Code\\Project\\AI-Vtuber-main\\models\\GPT-SoVITS-v2-240821\\ref_mood\\confused.mp3"
                    prompt_text = "呃…也不知道幸得和嘉九那边怎么样了，要不要过去看看呢？"
                    message["data"]["v2_api_0821"]["ref_audio_path"] = ref
                    message["data"]["v2_api_0821"]["prompt_text"] = prompt_text
                elif sense == "失落":
                    ref = "D:\\software\\Code\\Project\\AI-Vtuber-main\\models\\GPT-SoVITS-v2-240821\\ref_mood\\disappointed.mp3"
                    prompt_text = "一想到他以前冒失的样子，就有点担心。"
                    message["data"]["v2_api_0821"]["ref_audio_path"] = ref
                    message["data"]["v2_api_0821"]["prompt_text"] = prompt_text
                elif sense == "高兴":
                    ref = "D:\\software\\Code\\Project\\AI-Vtuber-main\\models\\GPT-SoVITS-v2-240821\\ref_mood\\happy.mp3"
                    prompt_text = "太好啦！嘿嘿，既解决了流浪武士的麻烦，又得到了大家的认可。"
                    message["data"]["v2_api_0821"]["ref_audio_path"] = ref
                    message["data"]["v2_api_0821"]["prompt_text"] = prompt_text
                elif sense == "惊讶":
                    ref = "D:\\software\\Code\\Project\\AI-Vtuber-main\\models\\GPT-SoVITS-v2-240821\\ref_mood\\surprised.mp3"
                    prompt_text = "唔！这么大的蛇果，看起来好吓人。"
                    message["data"]["v2_api_0821"]["ref_audio_path"] = ref
                    message["data"]["v2_api_0821"]["prompt_text"] = prompt_text
                elif sense == "其他":
                    ref = "D:\\software\\Code\\Project\\AI-Vtuber-main\\models\\GPT-SoVITS-v2-240821\\ref_mood\\normal.mp3"
                    prompt_text = "我们回去向心海汇报吧。"
                    message["data"]["v2_api_0821"]["ref_audio_path"] = ref
                    message["data"]["v2_api_0821"]["prompt_text"] = prompt_text
