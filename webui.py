            ...
            if True:
                ...
                config_mapping = {
                    "talk": {
                        ...
                        "device_index_out": (select_out_device_index, 'str'),
                        "device_name_out": (select_out_device_index, 'str'),
    ...
    with ui.tab_panels(tabs, value=common_config_page).classes('w-full'):
        with ui.tab_panel(common_config_page).style(tab_panel_css):
            with ui.row():
                ...
                input_room_display_id = ui.input(label='直播间号', placeholder='一般为直播间URL最后/后面的字母或数字', value=config.get("room_display_id")).style("width:200px;").tooltip('一般为直播间URL最后/后面的字母或数字t')
        ...
        with ui.tab_panel(talk_page).style(tab_panel_css): 
            ...
            with ui.row():
                ...
                audio_device_info_list_out = common.get_all_audio_device_info("out")
                # logger.info(f"声卡输出设备={audio_device_info_list_out}")
                audio_device_info_dict_out = {str(device['device_info']): device['device_info'] for device in audio_device_info_list_out}

                logger.debug(f"声卡输出设备={audio_device_info_dict_out}")

                select_out_device_index = ui.select(
                    label='声卡输出设备', 
                    options=audio_device_info_dict_out, 
                    value=config.get("talk", "device_index_out")
                ).style("width:300px;").tooltip('这就是语言对话输出的声卡，如果需要监听电脑声卡可以配合虚拟声卡来实现')
        ...
        with ui.tab_panel(data_analysis_page).style(tab_panel_css):
            ...
            with data_analysis_comment_word_cloud_card.style("width:100%;"):
                comment = data_analysis.get_comment_word_cloud_option(
                    int( config.get("data_analysis", "comment_word_cloud", "top_num") )
                    )
                if comment:
                    echart_comment_word_cloud = ui.echart(comment).style(echart_css)
            ...
            with data_analysis_integral_card.style("width:100%;"):
                comment = data_analysis.get_integral_option(
                    "integral", int(config.get("data_analysis", "integral", "top_num")))
                if comment:
                    echart_integral = ui.echart(comment).style(echart_css)
